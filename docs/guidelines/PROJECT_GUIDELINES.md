...
- El catálogo de menú se resuelve con `useMenuCatalog` (`app/menu/_hooks/use-menu-catalog.ts`); reutilizarlo para vistas relacionadas y usar su `refetch` en lugar de duplicar fetches.
...
- Las APIs de menú deben incluir los headers `x-menu-version` y `x-menu-updated-at` en todas las respuestas GET/HEAD para control de versiones en tablets y flujo QR.
